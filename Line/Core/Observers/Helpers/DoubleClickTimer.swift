...
final class DoubleClickTimer {
    ...
    /// - Parameter openCallback: A closure called when a double-press is detected. Receives the associated `BoundWindowAction`.
    ...
    /// - Parameter action: The `BoundWindowAction` associated with this key down.
    func handleKeyDown(startingAction: BoundWindowAction) {
        let now = Date()
