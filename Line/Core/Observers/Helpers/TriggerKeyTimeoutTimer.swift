...
//  Line
//
//  Created by nnecec on 06/01/2026.
//

import Defaults
import Foundation

/// A utility class that automatically closes Line if no action is taken within a specified timeout period.
///
/// This timer starts when Line is opened and automatically triggers the close callback if the user
/// doesn't perform any action within the configured timeout duration (defined by `Defaults[.triggerKeyTimeout]`).
/// The timer can be canceled if the user performs an action or manually closes Line.
final class TriggerKeyTimeoutTimer {
    ...
    /// Starts the timeout timer. If Line is still open when the timeout elapses, the close callback is invoked.
    ///
    /// This should be called when Line is opened. If a timeout is already active, it will be canceled and restarted.
    func start() {
