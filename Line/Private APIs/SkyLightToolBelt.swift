...
//  Line
//
//  Created by nnecec on 2025-11-24.
//
...
enum SkyLightToolBelt {
    ...
    static func makeKeyWindow(windowID: CGWindowID, pid: pid_t) -> Bool {
        ...
        for byte in [0x01, 0x02] {
            ...
            // - Apply an identifier to the event to help Line differentiate events that originate from itself
    ...
    /// Checks if the current window in a `SLSWindowIterator` is valid for Line to use.
