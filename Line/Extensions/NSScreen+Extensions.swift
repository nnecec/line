...
extension NSScreen {
    ...
    var displayBounds: CGRect {
        guard let displayID else {
            Log.error(
                "Failed to get NSScreen.displayID in NSScreen.displayBounds",
                category: LogCategory("NSScreen")
            )
    ...
    /// Stable identifier for grid configuration per screen.
    /// Uses UUID from displayID if available, falls back to displayID string.
