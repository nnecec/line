...
enum RectangleTranslationLayer {
    /// Maps Rectangle direction keys to Line's ProportionalLayout enum.
    private static let directionMapping: [String: ProportionalLayout] = [
    ...
    static func importKeybinds(from data: Data) throws -> [SavedWindowActionFormat] {
        ...
        // Converts the Rectangle shortcuts into Line's WindowActions.
        let windowActions: [SavedWindowActionFormat] = rectangleConfig.shortcuts.compactMap { direction, shortcut -> SavedWindowActionFormat? in
            guard
            ...
            return SavedWindowActionFormat(BoundWindowAction(
                action: .standard(.proportional(loopDirection)),
