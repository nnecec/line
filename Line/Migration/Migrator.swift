...
//  Line
//
//  Created by nnecec on 2024-03-22.
//
...
/// Struct to represent the JSON contents of a Line keybinds file.
...
/// This maintains backward compatibility with the legacy export format.
struct SavedWindowActionFormat: Codable {
    ...
    let stashEdge: StashEdge?

    /// Initialize from a BoundWindowAction.
    /// Extracts legacy-compatible properties from the WindowAction enum.
    init(_ boundAction: BoundWindowAction) {
        self.keybind = boundAction.keybind

        // Extract properties based on the WindowAction case
        switch boundAction.action {
        case let .custom(custom):
            self.direction = .custom
            self.name = custom.name
            self.unit = custom.unit
            self.anchor = custom.anchor
            self.sizeMode = custom.sizeMode
            self.width = custom.width
            self.height = custom.height
            self.positionMode = custom.positionMode
            self.xPoint = custom.xPoint
            self.yPoint = custom.yPoint
            self.cycle = nil
            self.stashEdge = nil
        ...
        case let .cycle(actions):
            ...
            self.width = nil
            self.height = nil
            self.positionMode = nil
            self.xPoint = nil
            self.yPoint = nil
            // Convert nested actions to SavedWindowActionFormat
            self.cycle = actions.map { SavedWindowActionFormat(BoundWindowAction(action: $0, keybind: [])) }
            self.stashEdge = nil

        case let .stash(stashName, edge):
            self.direction = .stash
            self.name = stashName
            self.unit = nil
            self.anchor = nil
            self.sizeMode = nil
        ...
        default:
            // For all other actions, use the legacy direction mapping
            self.direction = boundAction.action.legacyExportDirection
            self.name = nil
            self.unit = nil
            self.anchor = nil
            self.sizeMode = nil
    ...
    /// Converts the saved format back into a usable BoundWindowAction object.
    /// This handles importing from legacy formats.
    func convertToWindowAction() -> BoundWindowAction {
        // The actual conversion logic is handled by BoundWindowAction's Codable implementation
        // which knows how to decode from this legacy format.
        // This is a temporary bridge until we fully migrate away from this intermediate format.

        // For now, we construct a BoundWindowAction using its Codable decoder
        let encoder = JSONEncoder()
        let decoder = JSONDecoder()

        do {
            let data = try encoder.encode(self)
            let boundAction = try decoder.decode(BoundWindowAction.self, from: data)
            return boundAction
        } catch {
            // Fallback: create a no-action if conversion fails
            return BoundWindowAction(action: .special(.noAction), keybind: keybind)
        }
    }
}
...
private extension Migrator {
    ...
    static func getSaveDirectoryURL() async throws -> URL {
        ...
        savePanel.nameFieldStringValue = "Line Keybinds.json"
...
private extension Migrator {
    ...
    private static func importKeybinds(from jsonString: String, onSuccess: () -> ()) async throws {
        ...
        // First, try to import the general Line keybinds format.
        do {
            let savedData = try importLineKeybinds(from: data)
            await updateDefaults(with: savedData, onSuccess: onSuccess)
            return
        } catch {
            log.error("Error importing Line keybinds: \(ApplicationLogPrivacy.errorDescription(error))")
        }

        // If that fails, try to import the old Line (pre 1.2.0) keybinds format.
        do {
            let savedData = try importLineLegacyKeybinds(from: data)
            await updateDefaults(with: savedData, onSuccess: onSuccess)
            return
        } catch {
            log.error("Error importing Line (pre 1.2.0) keybinds: \(ApplicationLogPrivacy.errorDescription(error))")
        }
        ...
        do {
            ...
            log.error("Error importing Rectangle keybinds: \(ApplicationLogPrivacy.errorDescription(error))")
        }
...
extension Migrator {
    /// Tries to import Line's keybinds format.
    static func importLineKeybinds(from data: Data) throws -> SavedKeybindsFormat {
        let decoder = JSONDecoder()
        let keybinds = try decoder.decode(SavedKeybindsFormat.self, from: data)
        return keybinds
    }

    /// Tries to import Line's old (pre 1.2.0) keybinds format.
    static func importLineLegacyKeybinds(from data: Data) throws -> SavedKeybindsFormat {
        let decoder = JSONDecoder()
    ...
    static func updateDefaults(
        with savedData: SavedKeybindsFormat,
        onSuccess: () -> (),
        importDecision: (() async -> ImportDecision)? = nil
    ) async {
        ...
        if Defaults[.keybinds].isEmpty {
            ...
            let result: ImportDecision = if let importDecision {
                await importDecision()
            } else {
                await showAlertForImportDecision()
            }
            ...
            case .merge:
                let newKeybinds = savedData.actions
                    ...
                    .filter { newKeybind in
                        !Defaults[.keybinds].contains { existingKeybind in
                            existingKeybind.keybind == newKeybind.keybind &&
                                existingKeybind.displayName == newKeybind.displayName
                        }
                    }
