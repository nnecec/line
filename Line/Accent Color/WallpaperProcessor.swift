...
//  Line
//
//  Created by nnecec on 27/06/2024.
//
...
/// Processes desktop wallpapers to extract colors for theming Line.
...
final class WallpaperProcessor {
    ...
    private func fetchLatestWallpaperColors() async -> (primary: Color, secondary: Color)? {
        do {
            ...
            log.info("Successfully calculated dominant colors from wallpaper")

            return (primaryColor, secondaryColor)
        } catch {
            // If an error occurs, print the error description.
            log.error("Failed to fetch wallpaper colors: \(ApplicationLogPrivacy.errorDescription(error))")
            return nil
        }
    }
