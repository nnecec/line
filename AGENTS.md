...
主要 SwiftPM 依赖包括 Defaults、Scribe 和 Sparkle。Scribe 使用精确 revision。升级依赖时必须检查许可证、安全公告、macOS 26 兼容性，并提交更新后的 `Package.resolved`。
